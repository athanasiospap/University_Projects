...
class oil_burner{
  float max_P,fuel_rate,fuel_cost,machine_cost,service_cost;
  int days;
  char Obj_id[11];
  static char *C_id;
public:
  ...
  float get_op_cost(int days){return service_cost+fuel_rate*fuel_cost*days;}
...
{
cout<<"\nDwse kausthra (max 10 χαρακτήρες) = ?";
cin>>Obj_id;
cout<<"MEgisth isxus = ?";
cin>>max_P;
cout<<"Dwse katanalwsh = ?";
cin>>fuel_rate;
cout<<"Dwse timh petrelaiou ana litro = ?";
cin>>fuel_cost;
cout<<"Dwse kostos kausthra = ?";
cin>>machine_cost;
cout<<"Dwse kostos synthrhshs = ?";
cin>>service_cost;
}


class gas_burner{
  float max_P,fuel_rate,fuel_cost,machine_cost,service_cost,connection_cost;
  int days;
  char Obj_id[11];
  static char *C_id;
public:
...
{
  cout<<"\nDwse arithmo kaysthra = ? ";
  cin>>n;
  B=new burner[n];
  if(B==0){
    cout<<"den yparxei mnhmh 1\n";
   // exit(1);
  }
}
...
char *choice(burner1 *b1,int n1,burner2 *b2,int n2,float &p_out,float min_P,float max_c,int days)
{
    ...
    for(i=0;i<n1;i++)
        if(b1[i].get_maxP()>=min_P && b1[i].get_cost()<=max_c)
            break;
    for(j=0;j<n2;j++)
        if(b2[j].get_maxP()>=min_P && b2[j].get_cost()<=max_c)
            break;
    if(i==n1&&j<n2){
        return b2[i].get_obj_id();
    }
    if(i<n1&&j==n2){
...
int main()
{
    ...
    cout<<"Elaxisth isxus = ? ";
    cin>>min_P;
    cout<<"Megisto kostos agoras kai kaysthra = ? ";
    cin>>max_c;
    cout<<"Dwse arithmo hmerwn leitoyrgias = ? ";
    cin>>days;
    oil_burner *ob;
    gas_burner *gb;
